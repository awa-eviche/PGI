...
{!! Form::model(new \App\Models\FrontSlider(), ['route' => ['update.slider',$slider->id], 'role' => 'form', 'class' => 'apix-form', 'files' => 'true']) !!}
    @php
        $pClass = " p-2.5";
            ...
            <div class="md:container md:mx-auto">
                ...
                <div class="grid md:grid-cols-2 md:gap-6 pt-2">
                    <div class="relative z-0 w-full mb-3 group">
                        ...
                        {!! Form::text('slider_titre', $slider->slider_titre, ['id' => 'slider_titre', 'class' => $inputClass.$pClass, 'required' => 'true', 'placeholder' => 'Titre']) !!}
                    </div>
                    <div class="relative z-0 w-full mb-3 group">
                        <label for="prenom" class="block mb-2 text-sm font-medium text-regal-black dark:text-regal-black">Statut</label>
                        {!! Form::select('slider_is_active',[0=>'Inactif',1=>'Actif'], $slider->slider_is_active, ['id' => 'slider_is_active', 'class' => $inputClass.$pClass, 'required' => 'true']) !!}
                    </div>
                    <div class="relative z-0 w-full mb-6 group">
                        <label for="nom" class="block mb-2 text-sm font-medium text-regal-black dark:text-regal-black">Slider image</label>
                        {!! Form::file('slider_img', ['id' => 'slider_img', 'class' => $inputClass , 'placeholder'=>'Choisir une image']) !!}
                        @if(!empty($slider->slider_img))
                            <img class="thumbnail-slider py-4" src="{{ asset('storage/front/sliders/'. $slider->slider_img) }}" alt="{{$slider->slider_titre}}" />
                        @endif
                    </div>
                </div>
