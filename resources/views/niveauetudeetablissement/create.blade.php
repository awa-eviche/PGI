<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Nouveau programme de formation") }}
        </h2>
    </x-slot>

    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        <p>
            <a href="{{route('niveauetudeetablissement.index')}}" class="text-maquette">Accueil</a>
            <span class="mx-2 text-maquette">/</span>
            <a href="{{route('niveauetudeetablissement.index')}}" class="text-maquette">Programme de formation</a>
    ...
    <div class="rounded-sm w-full">
        ...
        <div class="mx-auto max-w-5xl shadow-xl rounded">
            <form action="{{ route('niveauetudeetablissement.store') }}" method="POST" class="bg-white border-x-2 rounded px-8 pt-6 pb-8 mb-4">
                @csrf
