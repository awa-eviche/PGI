...
@section('content')
    ...
    {!! Form::model(new \App\Models\Actualite(), ['route' => ['actualite.update',$actualite->id], 'method' => 'PUT', 'role' => 'form', 'class' => 'apix-form', 'files' => 'true']) !!}
    @php
        $pClass = " p-2.5";
        $tclass = " p-2.5";
    ...
    <div class="mt-4 mx-2 my-2">
        <a href="{{ route('actualite.index') }}" class="text-blue-500 hover:underline">&larr; Retour à la
            liste des
            actualités</a>
    </div>
    @method('PUT')
            ...
            <div class="md:container md:mx-auto">
                ...
                <div class="grid md:grid-cols-1 md:gap-6 pt-2">
                    <div class="relative z-0 w-full mb-6 group">
                        ...
                        {!! Form::text('title', $actualite->title, ['id' => 'title', 'class' => $inputClass.$pClass, 'required' => 'true', 'placeholder' => 'Titre']) !!}
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-1 md:gap-6 pt-2">
                    <div class="relative z-0 w-full mb-6 group">
                        <label for="main_pic"
                            ...
                            principale</label>
                        {!! Form::file('image', ['id' => 'image', 'class' => $inputClass.$tclass, 'placeholder'=>'Choisir une image']) !!}
                        @if(!empty($actualite->image))
                            <img class="thumbnail-slider py-4" src="{{ asset('storage/'. $actualite->image) }}"
                                 alt="{{$actualite->activite_main_pic}}"/>
                        @endif
                    </div>
                </div>
                <div class="grid md:grid-cols-1 md:gap-6 pt-2">
                    <div class="relative z-0 w-full mb-6 group">
                        ...
                        {!! Form::text('contenu', $actualite->content, ['id' => 'richEd', 'class' => $inputClass.$pClass, 'required' => 'true', 'placeholder' => 'Titre']) !!}
                    </div>
                </div>

                <div class="relative z-0 w-full mb-3 group">
                    <label for="status" class="block mb-2 text-sm font-medium text-regal-black dark:text-regal-black">Status</label>
                    {!! Form::select('status',['published'=>'Publié','draft'=>'Non publié'], $actualite->status, ['id' => 'status', 'class' => $inputClass.$pClass, 'required' => 'true']) !!}
                </div>
