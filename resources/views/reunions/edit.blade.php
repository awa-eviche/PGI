<x-app-layout>
    ...
    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        <p>
            <a href="{{ route('reunion.index') }}" class="text-maquette-gris">Liste des réunions</a>
        </p>
    ...
    <div class="rounded-sm w-full">
        <div class="mx-auto max-w-4xl shadow-xl rounded bg-white h-full pb-10">
            <form action="{{ route('reunion.update', $reunion->id) }}" method="POST" class=" border-x-2 rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                ...
                <div class="flex flex-wrap w-full justify-evenly mb-5">
                    ...
                    <div class="mb-4 mr-4 w-72">
                        ...
                        <select class="hadow appearance-none border-2 border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="charge_suivi_id" name="charge_suivi_id" required>
                            ...
                            @foreach($chargerSuivis as $chargerSuivi)
                                <option value="{{ $chargerSuivi->id }}">{{$chargerSuivi->id }} {{ $chargerSuivi->prenom }} {{ $chargerSuivi->prenom }}</option>
                            @endforeach --}}
                            @foreach($chargerSuivis as $chargerSuivi)
                                <option value="{{ $chargerSuivi->id }}" @if($chargerSuivi->id == $reunion->charge_suivi_id) selected @endif>
                                    {{ $chargerSuivi->id }} - {{ $chargerSuivi->prenom }} {{ $chargerSuivi->prenom }}
                                </option>
                            @endforeach
                        </select>
                        @error('charge_suivi_id')
                    ...
                    <div class="w-72 mr-4 mb-4">
                        ...
                        <x-input id="code" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" type="text" name="code" :value="old('code', $reunion->code)" required autofocus />
                        @error('code')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                    {{-- date prévue --}}
                    <div class="w-72 mr-4 mb-4">
                        <x-label for="date_prevue">
                            Date prévue <span class="text-red-500">*</span>
                        </x-label>
                        <x-input id="date_prevue" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" type="date" name="date_prevue" :value="old('date_prevue', $reunion->date_prevue)" required />
                        @error('date_prevue')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                    {{-- date effective --}}
                    <div class="w-72 mr-4 mb-4">
                        <x-label for="date_effective">
                            Date effective <span class="text-red-500"></span>
                        </x-label>
                        <x-input id="date_effective" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" type="date" name="date_effective" :value="old('date_effective', $reunion->date_effective)" />
                        @error('date_effective')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
