<x-app-layout>
    ...
    <div class="rounded-sm w-full">
        ...
        <div class="mx-auto max-w-3xl shadow-xl rounded">
            <form action="{{ route('reunion.store') }}" method="POST" class="bg-white border-x-2 rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                ...
                <div class="flex flex-wrap w-full justify-evenly">
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        <x-label for="date_prevue" value="{{ __('Date prévue') }}" class="font-semibold text-black" />
                        <x-input wire:model="donnees.date_prevue" id="date_prevue" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" type="date" name="date_prevue" :value="old('date_prevue')" autofocus autocomplete="date_prevue" />
                        @error('date_prevue')
                            <span class="text-xs text-red-500">
                                {{$message}}
