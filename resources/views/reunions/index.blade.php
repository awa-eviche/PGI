                    ...
                    <div>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex justify-end m-4 bg-first-orange hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded items-center">
                                <a href="{{ route('reunion.create') }}" class="mr-2 text-sm">
                                    Nouvelle reunion
                                </a>
                                ...
                                <tbody class="bg-white divide-y ">
                                    @foreach($reunions as $reunion)
                                        <tr class="text-gray-700 ">
                                            <td class="px-4 py-3 border-b">{{ $reunion->id ?? " " }}</td>
                                            <td class="px-4 py-3 border-b">{{ $reunion->code ?? "- " }}</td>
                                            <td class="px-4 py-3 border-b">{{date('d-m-Y',strtotime($reunion->date_prevue) ?? " ")}}</td>
                                            {{-- {{date('d/m/Y',strtotime($demandeDetails->date_depot))}} --}}
                                            <td class="px-4 py-3 border-b">{{date('d-m-Y',strtotime($reunion->date_effective) ?? " ")}}</td>
                                            <td class="px-4 py-3 border-b">{{ $reunion->chargeSuivi->prenom}} {{$reunion->chargeSuivi->nom ?? '' }}</td>
                                            ...
                                            <td class="px-2 py-3 border-b w-[60px]">
                                                <div class="flex justify-center rounded items-center space-x-2 text-sm shadow-xl p-0.5 bg-gray-100">
                                                    <a href="{{ route('reunion.show', $reunion->id) }}" class="text-maquette-gris">
                                                    ...
                                                    <a href="{{ route('reunion.edit', $reunion->id) }}" class="text-purple-600">
                                                    ...
                                                    <form class="text-center" action="{{ route('reunion.destroy', $reunion->id) }}" method="POST">
