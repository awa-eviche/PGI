<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Liste des Types de Notification') }}
        </h2>
    </x-slot>
    ...
    <div class="container p-2 rounded">
        <div class="flex justify-between items-center mb-3">
            ...
            <div class="flex justify-end">
                <a href="{{ route('type_notification.create') }}" class="flex bg-first-orange hover:bg-cyan-700 text-white font-bold py-1 text-sm px-4 rounded">
                    ...
                    <tbody class="bg-white divide-y ">
                        @foreach($typeNotifications as $typeNotification)
                            <tr class="text-gray-700">
                                <td class="px-4 py-3 border-b">{{ $typeNotification->action }}</td>
                                <td class="px-4 py-3 border-b">{{ $typeNotification->message }}</td>
                                <td class="px-4 py-3 border-b">
                                    <div class="flex justify-center rounded items-center space-x-2 text-sm shadow-xl p-0.5 bg-gray-100">
                                        <a href="{{ route('type_notification.show', $typeNotification->id) }}" class="text-maquette-gris">
                                        ...
                                        <a href="{{ route('type_notification.edit', $typeNotification->id) }}" class="text-purple-600">
                                        ...
                                        <form class="text-center" action="{{ route('type_notification.destroy', $typeNotification->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
