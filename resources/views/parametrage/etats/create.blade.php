<x-app-layout>
    ...
    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        ...
        <p>
            <a href="{{route("workflow.show", $workflow->id)}}"  class="text-first-orange">WF : {{$workflow->code}}</a>
        </p>
        <span class="mx-2 text-maquette-gris">/</span>
    ...
    <div class="bg-white shadow rounded-sm w-full">
        <div class="w-full mx-auto max-w-2xl">
            <form action="{{ route('etat_workflow.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
              @csrf
              ...
              <input type="hidden" name="workflow_id" value="{{ $workflow->id }}" />
                ...
                <div class="flex flex-wrap w-full justify-evenly">
                    <div class="flex-grow mb-4 mr-2 w-8">
                        ...
                        <x-input id="position" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="number" name="position" :value="old('position')" required autofocus autocomplete="position" />
                        @error('position')
                    ...
                    <div class="w-full mb-4 mr-4 w-96">
                        ...
                        <select class="shadow appearance-none border-2 border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="status" name="status" required>
                            <option value="{{App\Enums\StatusDemandeEnum::BROUILLON}}">Brouillon</option>
                            <option value="{{App\Enums\StatusDemandeEnum::COURS}}">En cours</option>
                            <option value="{{App\Enums\StatusDemandeEnum::REJETE}}">Rejeté</option>
                            <option value="{{App\Enums\StatusDemandeEnum::ATTENTE}}">Attente de complément</option>
                            <option value="{{App\Enums\StatusDemandeEnum::AVIS_FAVORABLE}}">Favorable</option>
                            <option value="{{App\Enums\StatusDemandeEnum::AVIS_DEFAVORABLE}}">Non favorable</option>
                            <option value="{{App\Enums\StatusDemandeEnum::VALIDE}}">Validé</option>
                        </select>
                    </div>
                ...
                <div class="flex flex-wrap w-full justify-evenly">
                    ...
                    <div class="flex-grow mb-4 mr-2 self-center">
                        ...
                        <div class="flex items-center">
                            <label class="mr-4">
                                <input type="radio" name="est_fin" value="1" class="form-radio text-blue-600" {{ old('est_fin') == '1' ? 'checked' : '' }}>
                                <span class="ml-2">OUI</span>
                            </label>
                            <label>
                                <input type="radio" name="est_fin" value="0" class="form-radio text-red-600" {{ old('est_fin') == '0' ? 'checked' : '' }}>
                    ...
                    <div class="flex-grow mb-4 mrl-2 self-center">
                        ...
                        <div class="flex items-center">
                            <label class="mr-4">
                                <input type="radio" name="est_rejetable" value="1" class="form-radio text-blue-600" {{ old('est_rejetable') == '1' ? 'checked' : '' }}>
                                <span class="ml-2">OUI</span>
                            </label>
                            <label>
                                <input type="radio" name="est_rejetable" value="0" class="form-radio text-red-600" {{ old('est_rejetable') == '0' ? 'checked' : '' }}>
                                <span class="ml-2">NON</span>
                            </label>
                        </div>
                    </div>
