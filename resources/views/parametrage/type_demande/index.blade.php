<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Liste des Types de Demandes') }}
        </h2>
    </x-slot>
    ...
    <div class="container p-2 rounded">
        <div class="flex justify-between items-center mb-3">
            ...
            <div class="flex justify-end">
                <a href="{{ route('type_demande.create') }}" class="flex bg-first-orange hover:bg-cyan-700 text-white font-bold py-1 text-sm px-4 rounded">
                    ...
                    <tbody class="bg-white divide-y">
                        @foreach($typeDemandes as $typeDemande)
                            <tr class="text-gray-700">
                                <td class="px-4 py-3 border-b">{{ $typeDemande->code }}</td>
                                <td class="px-4 py-3 border-b">{{ $typeDemande->libelle }}</td>
                                <td class="px-4 py-3 border-b">{{ $typeDemande->description }}</td>
                                <td class="px-4 py-3 border-b">
                                    <div class="flex justify-center rounded items-center space-x-2 text-sm shadow-xl p-0.5 bg-gray-100">

                                        <a href="{{ route('type_demande.show', $typeDemande->id) }}" class="text-maquette-gris">
                                        ...
                                        <a href="{{ route('type_demande.edit', $typeDemande->id) }}" class="text-purple-600">
                                        ...
                                        <form class="text-center" action="{{ route('type_demande.destroy', $typeDemande->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
