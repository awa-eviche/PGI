<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier le Type de Demande') }}
        </h2>
    </x-slot>
    ...
    <div class="bg-white shadow rounded-sm w-full">
        <div class="w-full mx-auto max-w-2xl">
            <form action="{{ route('type_demande.update', $typeDemande->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-wrap w-full justify-evenly">
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="code" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="code" value="{{ $typeDemande->code }}" required autofocus autocomplete="code" />
                        @error('code')
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="libelle" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="libelle" value="{{ $typeDemande->libelle }}" required autofocus autocomplete="libelle" />
                        @error('libelle')
                            <span class="text-xs text-red-500">
                                {{$message}}
                            </span>
                    ...
                    <div class="w-full mb-4 mr-4 w-96">
                        ...
                        <select class="shadow appearance-none border border-gray-100 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="type_demande_id" name="type_demande_id" required>
                            ...
                            @foreach($typeDemandeParents as $typeDemandeParent)
                              <option value="{{ $typeDemandeParent->id }}">{{ $typeDemandeParent->libelle }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-4 mx-auto w-full">
                    ...
                    <textarea value="{{ $typeDemande->description }}" class="enlever_shadow shadow appearance-none border-2 border-gray-400 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:border-first-orange" id="description" name="description" required rows="4" placeholder="Description">{{$typeDemande->description}}</textarea>
                    @error('description')
                        <span class="text-xs text-red-500">
                            {{$message}}
                        </span>
