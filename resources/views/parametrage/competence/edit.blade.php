<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modification de la compétence') }}
        </h2>
    </x-slot>
    ...
    <div class="mx-auto max-w-5xl shadow-xl rounded">
        <form action="{{ route('competence.update', $competence->id) }}" method="POST"
            ...
            <div class="border border-gray-200 p-4">
                ...
                {{-- Ligne : Code – Nom – Type (même ordre que création) --}}
                <div class="flex flex-wrap w-full justify-evenly">
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="code" name="code" type="text" required
                                 value="{{ old('code', $competence->code) }}" autofocus
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="nom" name="nom" type="text" required
                    ...
                    {{-- Type --}}
                    <div class="flex-grow mb-4 mr-2">
                        <x-label for="type">Type de compétence <span class="text-red-500">*</span></x-label>
                        <x-select id="type" name="type" required
                                  class="block w-full enlever_shadow rounded px-2 py-0.75 text-sm border-2 focus:border-first-orange">
                            <option value="" hidden disabled {{ old('type', $competence->type) ? '' : 'selected' }}>
                                Sélectionner un type
                            </option>
                            <option value="generale"
                                    {{ old('type', $competence->type) == 'generale' ? 'selected' : '' }}>
                                Générale
                            </option>
                            <option value="particuliere"
                                    {{ old('type', $competence->type) == 'particuliere' ? 'selected' : '' }}>
                                Particulière
                            </option>
                        </x-select>
                ...
                {{-- Composant Livewire (niveaux, métiers, etc.) --}}
