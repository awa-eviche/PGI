    ...
    <div class="flex flex-col sm:flex-row py-2">
            ...
            @if($selectedApprenant)
                <div class="sm:w-1/2 p-4 border bg-gray-100 rounded border shadow">
                    <h2 class="font-bold text-xl mb-4">Liste des compétences acquises par {{ $currentApprenant->apprenant->matricule}} {{ $currentApprenant->apprenant->nom}} {{ $currentApprenant->apprenant->prenom}}</h2>
                    <hr class="w-50">
                    <div>
...
@endphp
     @can('edit_competence')
    @if ($isFormateurAssigné && !auth()->user()->hasRole(config('constants.roles.superadmin')))
        <a class="text-white bg-blue-600 text-sm rounded-md shadow-md px-4 py-1"
           href="{{route('evaluate.create',$currentApprenant->id)}}">
    ...
    @if($competences && $competences->count() > 0)
        @foreach ($competences as $competence)
            ...
            <tr class="cptRow"
                ...
                wire:key="row-{{ $critere->id }}-sem-{{ $selectedsemestre ?? 'x' }}">
                {{-- COMPÉTENCE --}}
                <td class="px-4 py-3 border border-black font-bold">{{ $competence->nom ?? '-' }}</td>

                {{-- CRITÈRE (affiché une seule fois) --}}
                ...
                <td class="px-4 py-3 border border-black text-center">
                    <input type="number" min="0" max="20" step="0.5"
                           ...
                           value="{{ $evaluationData[$critere->id]['note'] ?? '' }}">
                </td>
                ...
                <td class="px-4 py-3 border border-black text-center">
                    <input type="date"
                           ...
                           value="{{ $evaluationData[$critere->id]['date'] ?? '' }}">
                </td>
            </tr>
            @endif
        @endforeach
