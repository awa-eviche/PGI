<div>
    <div class="flex mb-4 text-sm font-bold bg-white px-4 py-3 rounded-sm">
        ...
        <p><a href="{{route('actualite.index')}}">Page d'acceuil</a>
                ...
                @forelse ($actualites as $actualite)
                    <tr class="text-gray-700">
                        <td class="px-4 py-3 border-b">
                            <a target="_blank"
                               href="{{ asset('storage/'.$actualite->image) }}"><img
                                    class="w-20"
                        ...
                        <td class="px-4 py-3 border-b"><span
                                class="px-2 py-1 {{ ($actualite->status != "published") ? 'bg-red-100 text-red' : 'bg-green-100 text-green' }} rounded">{{$actualite->status == "published" ? 'Publié' : 'Non Publié' }}</span>
                        </td>

                        <td class="px-4 py-3 border-b ">{{ date('d-m-Y',strtotime($actualite->created_at)) ?? '-' }}</td>
                                    ...
                                    <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                        <div class="border shadow-md py-2 text-center">
                                            <form class="text-center"
                                                  action="{{ route('actualite.publier', $actualite->id) }}"
                                                  method="POST">
                                                @csrf
                                                @method('POST')
                                                <button title="Supprimer" type="submit" class="text-purple-600">
                                                    {{$actualite->status == "published" ? "Dépublier" : "Publier"}}
                                                </button>
                                            </form>
                                        </div>
                                        <div class="border shadow-md py-2 text-center">
                                            <a href="{{ route('actualite.edit', $actualite->id) }}"
