<div class="max-w-10xl mx-auto mt-5">
    <div class="w-full flex justify-between">
        ...
        @if ($typeUser === "DFPT" || auth()->user()->hasRole(config('constants.roles.ia')))
            <div class="">
    ...
    <div class="mx-auto max-w-5xl shadow-xl rounded">
        <form action="{{ route('update.document', $idDocument) }}" method="POST"
            ...
            <h3 class="bg-gray-100 p-2 text-sm font-bold text-first-orange">
                Modifier le document #{{ $idDocument }}
            </h3>

            <div class="border border-gray-200 p-4 space-y-6">
                {{-- Libellé --}}
                <div>
                    <x-label for="libelle">Libellé <span class="text-red-500">*</span></x-label>
                    <x-input id="libelle"
                             class="w-full md:w-96 focus:border-first-orange enlever_shadow rounded px-2 py-1 shadow-first-orange text-sm border-2"
                             type="text" name="libelle"
                             value="{{ old('libelle', $libelleDocument) }}" required />
                    @error('libelle')
                        <div class="text-xs text-red-500 mt-1">{{ $message }}</div>
                    @enderror
                </div>

                {{-- Fichier actuel (aperçu / lien) --}}
                @if ($currentDocUrl)
                    <div class="bg-gray-50 p-3 rounded border">
                        <div class="text-sm text-gray-700 mb-2">Fichier actuel :</div>
                        <div class="flex items-center gap-4">
                            <a href="{{ $currentDocUrl }}" target="_blank" class="text-blue-600 underline text-sm">
...
<div>
  <x-label for="document">Remplacer le fichier (optionnel)</x-label>
...
@endif
        ...
        <div class="flex justify-start bg-white border-x-2 rounded px-8 pt-6 pb-8 mb-4">
            <div class="w-1/4 h-full  text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                ...
                <div class="flex  justify-start ">
                        <a  class="element_sidebar selectCategory"  wire:click="getFileCategory({{ $categoryFile->id }})"
                        >
                    ...
                    <div class="flex items-start ">
                        ...
                        <div id="{{ $categoryFile->id }}" class="z-10 my-48 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-40 dark:bg-gray-700 dark:divide-gray-600">
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
                                    @if ($typeUser === "DFPT" || in_array("Ajouter", $categoryFile->access[$typeUser]))
                                    <li>
                                        <a data-modal-target="aa{{ $categoryFile->id }}" data-modal-toggle="aa{{ $categoryFile->id }}" {{--href="{{ route('show.category', $categoryFile->id) }}"  --}}  class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            Ajouter un document
                                        </a>
                                    </li>
                                @endif
    ...
    <!-- Modal toggle -->
        ...
        <!-- Main modal -->
        <div id="aa{{ $categoryFile->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="aa{{ $categoryFile->id }}">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                            </svg>
                            <span class="sr-only">Close modal</span>
                    ...
                    <!-- Modal body -->
                    <div class="p-4 md:p-5 space-y-4">
                        <form action="{{ route('crate.document') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="border border-gray-200 p-4">
                                <div class="flex flex-wrap w-full justify-between">
                                    <div class="flex-grow mb-4 mr-2">
                                        ...
                                        <x-input id="libelle" class=" w-96 focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="libelle" :value="old('libelle')" required autofocus autocomplete="libelle" />
                                        @error('libelle')
                                    ...
                                    <div class="flex-grow mb-4 mr-2">
                                            ...
                                            <x-input id="document" class=" w96 styled_input_file  focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="file" name="document" :value="old('document')" required autofocus autocomplete="document" />
                                            @error('document')
                                            <span class="text-xs text-red-500">
                                                {{$message}}
                                ...
                                <input type="hidden" name="category_file_id" value="{{$categoryFile->id}}">
                    ...
                    <div class="flex justify-end p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button data-modal-hide="aa{{ $categoryFile->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Fermer</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div  class="min-w-35 h-48 m-3 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700" type="button">
                                ...
                                <div class=" flex justify-evenly">
                                    ...
                                    <a href="#" wire:click="download('{{ $fichier["document"] }}', '{{ $fichier["libelle"]}}')">
                                        ...
                                        @if ($typeUser === "DFPT" || $fichier['user_id'] == auth()->user()->id)
