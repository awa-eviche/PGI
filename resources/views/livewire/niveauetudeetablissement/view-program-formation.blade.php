        ...
        <div class="w-full h-auto bg-white rounded-md shadow-md">
            ...
            <div class="px-3 py-2">
                <ul class="max-h-48 overflow-y-auto">
                    ...
                    @endphp
                    <span class="inline-block px-2 py-1 text-xs font-semibold {{ $statusClasses }} rounded-full">{{$label}}
                    </span>
                    <li class="border-b p-2 font-normal mb-1">
                        <i class="fa fa-circle text-xs me-2"></i>{{ $niv->nom ?? '-' }}
                    </li>
                    <a href="{{ route('program.validate', ['id' => $niv->id, 'decision' => 'false', 'idEtablissement' => $etablissement->id]) }}" title="Refuser le programme de formation" class="rounded-full px-1 py-1 text-white text-xs font-bold text-center bg-orange-500  hover:bg-orange-700">
                        <span class="mx-1"><i class="fa fa-times"></i></span>
                    </a>
                    <a href="{{ route('program.validate', ['id' => $niv->id, 'decision' => 'true', 'idEtablissement' => $etablissement->id]) }}" title="Valider le programme de formation" class="rounded-full ml-1 px-1 py-1 text-white text-xs font-bold text-center bg-blue-500  hover:bg-blue-700">
                        <span class="mx-1"><i class="fa fa-check"></i></span>
                    </a>
                    <a href="{{ route('program.logic.remove', ['id' => $niv->id, 'idEtablissement' => $etablissement->id]) }}" title="Supprimer le programme de formation" class="rounded-full ml-1 px-1 py-1 text-white text-xs font-bold text-center bg-red-500  hover:bg-red-700">
