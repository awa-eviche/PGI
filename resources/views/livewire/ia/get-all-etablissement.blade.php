<div>
    ...
    <tbody class="bg-white divide-y">
        @if ($etablissements && $etablissements->count() > 0)
            @foreach ($etablissements as $etablissement)
                <tr class="text-gray-700 hover:bg-gray-100">
                    <td class="p-2">{{ $etablissement->nom ?? '-' }}</td>
                    <td class="p-2">{{ $etablissement->email ?? '-' }}</td>
                    <td class="p-2">{{ $etablissement->nameCommune ?? '-' }}</td>
                    <td class="p-2">{{ $etablissement->type ?? '-' }}</td>
                    <td class="p-2">{{ $etablissement->reference ?? '-' }}</td>
                </tr>
            @endforeach
        @else
            <tr>
