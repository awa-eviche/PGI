<div>
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="{{route('create.actualite')}}" class="px-3 rounded-md py-3 flex text-white text-sm text-center bg-first-orange">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    @forelse ($actualites as $actualite)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 border-b">
                                <a target="_blank" href="{{ asset('storage/front/actualites/'. $actualite->actualite_main_pic) }}"><img class="thumbnail-slider" src="{{ asset('storage/front/actualites/'. $actualite->actualite_main_pic) }}" alt="avatar" /></a>
                            </td>
                            <td class="px-4 py-3 border-b text-maquette-gris">
                                {{ $actualite->actualite_titre ?? ' - ' }}</td>
                            <td class="px-4 py-3 border-b  font-bold"><span class="text-center px-2 py-1 {{ ($actualite->actualite_is_active == 0) ? 'text-red-600 bg-red-100' : 'text-green-600	bg-green-100' }}">{{$actualite->actualite_is_active ? 'Publié' : 'Dépublié' }}</span></td>

                            <td class="px-4 py-3 border-b text-maquette-gris">{{ date('d-m-Y',strtotime($actualite->created_at)) ?? '-' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="border shadow-md py-2 text-center">
                                                <form class="text-center"
                                                    action="{{ route('publier.actualite', $actualite->id) }}" method="POST">
                                                    @csrf
                                                    @method('POST')
                                                    <button title="Supprimer" type="submit" class="text-purple-600">
                                                        {{$actualite->actualite_is_active == true ? "Dépublier" : "Publier"}}
                                                    </button>
                                                </form>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('edit.actualite', $actualite->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
                                            ...
                                            <div class="border shadow-md py-2 text-center">
                                                <form class="text-center"
                                                    action="{{ route('destroy.actualite', $actualite->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button title="Supprimer" type="submit" class="text-purple-600">
                                                        Supprimer
