<div>
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="{{route('index.suscribers')}}" class="mx-2 px-3 rounded-md py-3 flex text-white text-sm text-center bg-first-orange">
            ...
            <a href="{{route('create.newsletters')}}" class="px-3 rounded-md py-3 flex text-white text-sm text-center bg-first-orange">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    @forelse ($newsletters as $newsletter)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 border-b">
                                {{ $newsletter->newsletter_object ?? ' - ' }}
                            </td>
                            <td class="px-4 py-3 border-b  font-bold"><span class="text-center px-2 py-1 {{ ($newsletter->newsletter_is_sent == 0) ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100' }}">{{$newsletter->newsletter_is_sent ? 'OUI' : 'NON' }}</span></td>
                            <td class="px-4 py-3 border-b text-maquette-gris">{{ date('d-m-Y',strtotime($newsletter->created_at)) ?? '-' }}</td>
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                    ...
                                    <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                        <div class="border shadow-md py-2 text-center">
                                            <a href="{{ route('edit.newsletters', $newsletter->id) }}" class="text-purple-600">
                                                Modifier
                                            </a>
                                        </div>
                                        <div class="border shadow-md py-2 text-center">
                                            <form class="text-center"
                                                action="{{ route('destroy.newsletters', $newsletter->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
