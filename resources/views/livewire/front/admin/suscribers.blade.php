        ...
        <div class="text-sm w-full p-0">
            <table class="w-full border-t mb-3">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @forelse ($suscribers as $suscriber)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 border-b">
                                {{ $suscriber->suscriber_email ?? ' - ' }}</td>
                            <td class="px-4 py-3 border-b  font-bold"><span class="text-center px-2 py-1 {{ ($suscriber->suscriber_is_active == 0) ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100' }}">{{$suscriber->suscriber_is_active ? 'Actif' : 'Inactif' }}</span></td>

                            <td class="px-4 py-3 border-b text-maquette-gris">{{ date('d-m-Y',strtotime($suscriber->created_at)) ?? '-' }}</td>

                            <td class="px-4 py-3 border-b text-end">
                                @if(!$suscriber->suscriber_is_active)
                                    <div class="border shadow-md py-2 text-center">
                                        <a href="{{ route('enable.suscribers', $suscriber->id) }}" class="text-purple-600">
                                            Activer
                                        </a>
                                    </div>
                                @else
                                    <div class="border shadow-md py-2 text-center">
                                        <a href="{{ route('disable.suscribers', $suscriber->id) }}" class="text-red-600">
                                            Désactiver
                                        </a>
                                    </div>
                                @endif
