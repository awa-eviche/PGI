<div>
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            @if(auth()->user()->can('edit_apprenant') and !auth()->user()->hasRole(config('constants.roles.superadmin')))
            <a href="{{route('apprenant.create')}}" class="px-3 rounded-md py-3 flex text-white text-sm text-center bg-first-orange">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                ...
                <tbody class="bg-white divide-y ">
                    ...
                    @forelse ($apprenants as $apprenant)
                        <tr class="text-gray-700 ">
                            ...
                            <td class="px-6 py-3 border-b">{{ $apprenant->prenom ?? ' Non renseigné ' }}</td>
                
                            <td class="px-6 py-3 border-b">{{ $apprenant->nom ?? ' Non renseigné' }}</td>
							
							<td class="px-6 py-3 border-b">{{ $apprenant->email ?? 'Non renseigné' }}</td>
							
							<td class="px-6 py-3 border-b">{{ $apprenant->telephone ?? 'Non renseigné' }}</td>
                            <td class="px-6 py-3 border-b">{{ $apprenant->sexe ?? 'Non renseigné' }}</td>
                            <td class="px-6 py-3 border-b">{{ $apprenant->commune->libelle ?? 'Non renseigné' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('apprenant.show', $apprenant->id) }}"
                                                    class="text-purple-600">Voir
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('apprenant.edit', $apprenant->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                    ...
                    {!! Form::close() !!}
                                ...
                                {{--  <div
                                    ...
                                    <form class="text-center"
                                        action="{{ route('apprenant.destroy', $apprenant->id) }}" method="POST">
