<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modification d\'un IEF') }}
        </h2>
    </x-slot>
    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        <p>
            <a href="{{route('ief.index')}}" class="text-maquette">Accueil</a>
            <span class="mx-2 text-maquette">/</span>
            <a href="{{route('ief.index')}}" class="text-maquette">Référentiel</a>
            <span class="mx-2 text-maquette">/</span>
        <p> <a href="{{route('ief.index')}}">ief </a>
    ...
    <div class="rounded-sm w-full p-4 mt-2">
        ...
        <div class="rounded-sm w-full">
            <div class="w-full mx-auto max-w-6xl">
                ...
                <form class="bg-white shadow-md rounded pt-6 pb-8 mb-4" action="{{ route('ief.update', $ief->id) }}" method="POST">
                    @csrf
                            ...
                            <div class="max-w-7xl sm:px-2 lg:px-4 ">
                                ...
                                <div class="flex flex-wrap w-full justify-evenly">
                                    <div class="flex-grow mb-4 mr-2">
                                        ...
                                        <x-input id="nom" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="nom" value="{{ $ief->nom }}" required autofocus autocomplete="nom" />
                                        @error('nom')
                                    ...
                                    <div class="flex-grow mb-4 mr-2">
                                        ...
                                        <x-input id="email" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="email" value="{{ $ief->email}}" required autofocus autocomplete="email" />
                                        @error('email')
                                        <span class="text-xs text-red-500">
                                            {{$message}}
                                ...
                                <div class="flex-grow mb-4 mr-2">
                                        ...
                                        <x-input id="telephone" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="telephone" value="{{ $ief->telephone}}" required autofocus autocomplete="telephone" />
                                        @error('telephone')
                                        <span class="text-xs text-red-500">
                                            {{$message}}
                                    ...
                                    <div class="flex-grow mb-4 mr-2">
                                        ...
                                        <x-input id="adresse" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="adresse" value="{{ $ief->adresse}}" required autofocus autocomplete="adresse" />
                                        @error('adresse')
                                        <span class="text-xs text-red-500">
                                            {{$message}}
                                ...
                                <div class="flex-grow mb-4 mr-2">
                                        ...
                                        <select class="shadow appearance-none border-2 border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="ia_id" name="ia_id" required>
                                            <option value="{{$ief->ia_id}}"><span class="text-red-500">{{$ief->ia->nom}}</span></option>
                                            @foreach($ias as $ia)
                    ...
                    <div class="checkbox-columns" style="column-count: 4; column-gap: 20px;">
                        ...
                        <div class="form-check" style="margin-bottom: 10px;">
                            <input class="form-check-input" type="checkbox" id="commune{{$commune->id}}" name="communes[]" value="{{$commune->id}}"   @if(in_array($commune->id, $ief->communes->pluck('id')->toArray())) checked @endif>
