    ...
    function generateCompetencePdf(string $id)
{
    ...
    $evaluations = Evalute::where('inscription_id', $inscription->id)->get()->keyBy('critere_id');

    $competences = Competence::where('niveau_etude_id', $inscription->classe->niveau_etude_id)
    ...
    foreach ($competences as $cptCompetence => $competence) {
        foreach ($competence->elementCompetences as $elementCompetence) {
            foreach ($elementCompetence->criteres as $cpt => $critere) {
                ...
                // Colonne compétence avec rowspan
                if ($rowspanCount == 0) {
                    $output .= '<td rowspan="' . $rowspans[$cptCompetence] . '" class="border-td">' . htmlspecialchars($competence->nom) . '</td>';
                }

                // Critère (si plusieurs par EC)
                $output .= '<td class="border-td">' . htmlspecialchars($critere->libelle) . '</td>';

                // Note
                $note = $evaluations->get($critere->id)->note ?? '-';
                $output .= '<td class="border-td" align="center">' . $note . '</td>';

                // Date
                $date = $evaluations->get($critere->id)->date ?? null;
                $output .= '<td class="border-td" align="center">';
                if ($date) {
                ...
                // Observation
                $obs = $evaluations->get($critere->id)->observations ?? '';
                $output .= '<td class="border-td" align="center">' . htmlspecialchars($obs) . '</td>';
